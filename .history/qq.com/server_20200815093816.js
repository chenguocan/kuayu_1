...
var server = http.createServer(function (request, response) {
    ...
    if (path === '/index.html') {
        ...
        const string = fs.readFileSync("./public/qq.js")
        response.write(string);
        response.end()
        ...
        const string = fs.readFileSync("./public/friend.json")
        response.write(string);
        response.end()
    } else if (path === "/friends.js") {
        if (request.headers["referer"].indexOf("http://frank.com:9999/index.html") === 0) {
            response.statusCode = 200
